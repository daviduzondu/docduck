...
export default function Comments() {
 ...
 const [comments, setComments] = useState<Record<string, Comment>>(
  provider.document.getMap('comments').toJSON()
 )
 const [activeCommentId, setActiveCommentId] = useState(
  editor?.storage.comment.activeCommentId
 )
...
function CommentCard({
 ...
 userData: Awaited<ReturnType<typeof $api.users.getBasicUserInfo>>
}) {
 ...
 const { mutate } = useMutation(
  orpc.documents.editComment.mutationOptions({
   onSuccess() {
    setIsEditing(false)
   ...
   onError(error) {
    ...
    toast.error('Failed to update comment', {
     description: isDefinedError(error)
      ? error.message
 ...
 return (
  <Card
   ...
   className={`cursor-pointer transition-all ${
    isActive ? 'outline-[3px] outline-primary bg-primary/10' : 'bg-transparent'
   } p-2 rounded-xl`}
  >
   <CardContent className="p-1 flex flex-col space-y-3">
    <div className="flex items-center justify-between gap-2">
     ...
     <CommentDropdownMenu
      ...
      canEdit={comment.commenterId === data?.user?.id}
     />
    </div>
    ...
    <InputGroup
     className={cn(
      'has-data-[align=block-end]:rounded-lg has-[textarea]:rounded-lg'
     )}
    >
     <InputGroupTextarea
      ref={textAreaRef}
