...
export async function addDocInvitees(
 documentId: string,
 invitees: Pick<
  Insertable<document_invitation>,
  'email' | 'inviterId' | 'role'
 >[],
 user: User,
 ...
 const results = await db
  .insertInto('document_invitation')
  .values(invitees.map((i) => ({ ...i, documentId })))
  .returning((eb) => [
   'email',
   'status',
   'document_invitation.id',
   'document_invitation.createdAt',
   'document_invitation.updatedAt',
  ])
  .onConflict((oc) => {
   return oc
    .columns(['email', 'documentId'])
    .where((eb) =>
     eb
      .or([eb('status', '=', 'PENDING'), eb('status', '=', 'ACCEPTED')])
 ...
 return {
  ...
  skipped: invitees.filter(
   (i) => !results.map((r) => r.email).includes(i.email)
  ),
 }
}
...
export async function getInvitationDetails(invitationId: string) {
 return await db
  ...
  .select(['id', 'role', 'inviterId', 'status', 'email'])
  .where('id', '=', invitationId)
  .executeTakeFirstOrThrow(() => {
   throw new AppError('Failed to retrieve invitation', StatusCodes.NOT_FOUND)
  })
}
...
export async function acceptDocumentInvitation(
 ...
 return await db.transaction().execute(async (trx) => {
  const invitation = await trx
   .selectFrom('document_invitation')
   .where('document_invitation.id', '=', invitationId)
   .select([
    'documentId',
    'email',
    'role',
    'status',
    'document_invitation.revokedAt',
   ])
   .executeTakeFirstOrThrow(() => {
  ...
  if (invitation.revokedAt !== null) {
   throw errors.FORBIDDEN({
    message:
     'You cannot accept this invitation because this link has been revoked.',
   })
  }

  const updateTableResult = await trx
