...
type DocumentRow = Awaited<
 ReturnType<typeof $api.documents.getSharedDocuments>
>['data'][number]
...
function useDocumentActions(row: Row<DocumentRow>) {
 ...
 return { handleRename, isUpdatingTitle }
}

function ActionsMenu({ row }: { row: Row<DocumentRow> }) {
 const { handleRename, isUpdatingTitle } =
...
export default function Shared() {
 ...
 const { data, isLoading } = useQuery(
  orpc.documents.getSharedDocuments.queryOptions({
   input: {
 ...
 const { setMaxPages, maxPages, searchTerm, view, activeFilters, sortBy } =
  useDashboard(
   useShallow((state) => ({
    ...
    maxPages: state.shared.maxPages,
    searchTerm: state.shared.searchTerm,
    view: state.shared.view,
    activeFilters: state.shared.activeFilters,
 ...
 if (data) {
  const maxPages = Math.ceil(
   Number(data?.data[0]?.total ?? 0) / Number(data.data[0]?.limit ?? 0)
  )
  setMaxPages('shared', isNaN(maxPages) ? 1 : maxPages)
 }
 ...
 return (
  <DashboardShell
   ...
   pageName="shared"
   selector={(state) => state.shared}
   getPrevPage={table.previousPage}
   getNextPage={table.nextPage}
   pageCount={table.getPageCount}
   onSetActiveFilters={(f: DocumentFilter[]) =>
    useDashboard.getState().setActiveFilters('shared', f)
   }
   onToggleFilter={(f) => useDashboard.getState().toggleFilter('shared', f)}
   onClearFilters={() =>
    useDashboard.getState().setActiveFilters('shared', [])
   }
   onSortChange={(s) => useDashboard.getState().setSortBy('shared', s)}
  >
   {view === 'list' && (
