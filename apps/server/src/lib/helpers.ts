...
export const validateRequest =
 (schema: { query?: z.ZodType; body?: z.ZodType; params?: z.ZodType }) =>
...
export async function verifyRole({
 ...
 if (document.visibility === 'PRIVATE' && !userId)
  throw new AppError(
 ...
 const permission = await db
  ...
  .select(['documentId', 'role', 'userId'])
  .where('userId', '=', userId)
  .executeTakeFirstOrThrow(() => {
   throw new AppError(
    'You do not have permission to view this document. Contact the document owner.',
    StatusCodes.UNAUTHORIZED
   )
  })
