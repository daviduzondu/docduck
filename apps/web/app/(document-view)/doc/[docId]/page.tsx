...
export default async function Page({
 ...
 try {
  ...
  console.log(result)
  if (!result?.meta.documentId)
   return <div>Hmm... This document does not exists.</div>
  if (!result.permissions.canView && session.data)
   return <div>Hmmm... You need permission to view that...</div>
  if (!result.permissions.canView && !session.data)
   return <div>Hmm...You need to be signed in first.</div>
 } catch (error) {
