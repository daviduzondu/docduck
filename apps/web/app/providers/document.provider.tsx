...
export const useDocumentStore = createStore<DocumentState & DocumentActions>(
 (set) => ({
  title: undefined,
  ...
  initialize(documentId, title) {
   ...
   const provider = new HocuspocusProvider({
    ...
    onAuthenticationFailed(data) {
     console.error(
      'Authentication failed. You must be signed in to perform this action.'
     )
    },
    onStateless({ payload }) {
...
export function useDocument<T = DocumentReadyState & DocumentActions>(
 ...
 if (!ydoc || !provider)
  throw new Error('useDocument must be called within a DocumentProvider.')

 return useStore(
  useDocumentStore,
  selector
   ? (state) =>
      selector(state as unknown as DocumentReadyState & DocumentActions)
