...
export const documentRouter = base
 ...
 .router({
  ...
  getSharedDocuments: r
   ...
   .use(ensureAuth)
   .handler(({ context, input }) =>
    documentService.getSharedDocuments(context.user.id, input.query?.page)
   ),
  ...
  getDocument: r
   ...
   .handler(({ input }) => documentService.getDocument(input.params)),

  getDocumentWithPermissions: r
   .get('/{documentId}/permissions', { inputStructure: 'detailed' })
   .input(
    z.object({
     params: z.object({ documentId: z.string() }),
    })
  ...
  getSnapshots: r
   .get('/{documentId}/snapshots', { inputStructure: 'detailed' })
   .input(
    z.object({
     params: z.object({ documentId: z.string() }),
     query: z.object({ page: z.coerce.number().optional() }).optional(),
  ...
  getSnapshotById: r
   ...
   .use(ensureCanEditDocument, (input) => input.params.documentId)
   .handler(({ input }) =>
    documentService.getSnapshotById({
     snapshotId: input.params.snapshotId,
     documentId: input.params.documentId,
    })
   ),

  restoreSnapshotbyId: r
   ...
   .use(ensureCanEditDocument, (input) => input.params.documentId)
   .handler(({ input }) =>
    documentService.restoreSnapshotById(
  ...
  addNewComment: r
   .post('/{documentId}/comments', { inputStructure: 'detailed' })
   .input(
    z.object({
     params: z.object({
      documentId: z.string(),
  ...
  editComment: r
   ...
   .use(ensureCanEditDocument, (input) => input.params.documentId)
   .use(ensureAuth)
   .use(ensureCommentAuthor, (input) => ({
    documentId: input.params.documentId,
    commentId: input.params.commentId,
   }))
   .handler(({ input }) =>
    documentService.editComment({
     text: input.body.text,
     documentId: input.params.documentId,
     commentId: input.params.commentId,
    })
  ...
  getCollaborators: r
   .get('/{id}/collaborators', { inputStructure: 'detailed' })
  ...
  createDocumentInvitations: r
   .post('/{id}/invitations', { inputStructure: 'detailed' })
   .input(documentSchema.documentInvitationSchema)
   .use(ensureAuth)
   .use(ensureDocumentOwner, (input) => input.params.id)
   .handler(({ input, context, errors }) =>
    invitationService.addDocInvitees(
     input.params.id,
     input.body.invitees.map((i) => ({
      ...i,
