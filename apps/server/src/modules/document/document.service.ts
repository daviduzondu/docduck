...
export async function getDocumentWithPermissions(
 ...
 const result = await db
  .selectFrom('document')
  .leftJoin('permission', (join) =>
   join
    .onRef('permission.documentId', '=', 'document.id')
    .on((eb) =>
     eb('permission.userId', '=', userId).or('permission.userId', 'is', null)
    )
  )
...
export async function getSnapshotById({
 ...
 return {
  ...result,
  yjsState: fromUint8Array(Y.encodeStateAsUpdate(snapshotDoc)),
 }
}
    ...
    eb
     .or([
      ...
      eb(
       ...
       '<',
       sql<Date>`now() - interval '00:15:00'`
      ),
     ])
...
function revertToSnapshot(liveDoc: Y.Doc, snapshotState: Uint8Array) {
 ...
 const changesSinceSnapshot = Y.encodeStateAsUpdate(
  liveDoc,
  snapshotStateVector
 )
...
export async function restoreSnapshotById(
 ...
 return await db.transaction().execute(async (trx) => {
  const snapshot = await trx
   .selectFrom('document_snapshot')
   .select([
    'document_snapshot.creatorId',
    'document_snapshot.id',
    'name',
    'document_snapshot.yjsState',
   ])
   .where('document_snapshot.documentId', '=', documentId)
   .where('id', '=', snapshotId)
   .executeTakeFirstOrThrow()
...
export async function addNewComment({
 ...
 if (hocuspocusDocument) {
  ...
  throw new AppError(
   "You're not connected to any document",
...
export async function getDocument(
 data: z.infer<typeof getDocumentSchema>['params']
) {
 return await db
  .selectFrom('document')
  .where('document.id', '=', data.documentId)
  .select(['id', 'title', 'visibility', 'ownerId'])
  .executeTakeFirstOrThrow()
}
...
export async function createDocument(
 ...
 return await db.transaction().execute(async (trx) => {
  ...
  const { role } = await trx
   ...
   .executeTakeFirstOrThrow(() => {
    throw new AppError(
     'An error occured when generating permissions',
     StatusCodes.INTERNAL_SERVER_ERROR
    )
   })
