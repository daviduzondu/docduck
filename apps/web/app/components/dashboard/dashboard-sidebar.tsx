...
function ThemeToggle() {
 ...
 return (
  <DropdownMenu>
   <DropdownMenuTrigger
    render={
     <SidebarMenuButton tooltip="Toggle theme" className="w-full">
      ...
      <SidebarMenu>
       {navItems.map((item) => (
        <SidebarMenuItem key={item.label}>
         <SidebarMenuButton
          className={`${pathname === item.href && 'border bg-accent-foreground/20'}`}
          render={
           <Link href={item.href}>
            <item.icon />
       ...
       render={
        <Button
         ...
         onClick={async () => {
          ...
          if (signOutConfirmed) await authClient.signOut()
         }}
        >
         Sign out
        </Button>
