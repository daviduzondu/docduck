...
export async function createDocumentInvitations(...[req, res]: MiddlewareArgs<z.infer<typeof documentSchema.documentInvitationSchema['params']>, {}, z.infer<typeof documentSchema.documentInvitationSchema['body']>>) {
 const result = await invitationsService.addDocInvitees(req.params.id, req.body.invitees.map(x => ({
  ...x,
