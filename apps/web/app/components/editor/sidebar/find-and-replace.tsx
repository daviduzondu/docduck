...
export default function FindAndReplace({ canEdit }: { canEdit: boolean }) {
 ...
 useEffect(() => {
  ...
  if (open && searchInputRef)
   editor?.commands.setSearchTerm(searchInputRef.current!.value)
 }, [open])
      ...
      <Toggle
       ...
       render={(props, state) => (
        <Button
         ...
         variant={state.pressed ? 'default' : 'ghost'}
         size="icon-sm"
        >
         <LetterText />
        </Button>
     ...
     <Empty className="flex-1 justify-center">
      <EmptyHeader>
       ...
       <EmptyDescription>
        ...
        <span className="text-foreground font-medium">Regex</span> to refine
        your search.
        {canEdit ? (
         <p className="mt-4">
          {' '}
          Enter a replacement term and press{' '}
          <span className="text-foreground font-medium">Enter</span> to replace
          one, or enable{' '}
          <span className="text-foreground font-medium">Replace All</span> to
      ...
      {editor &&
       results.map((r: any, i: number) => {
        ...
        return (
         <Card
          key={`${r.from}-${r.to}-${i}`}
          className="rounded-xl shrink-0 cursor-pointer p-2"
          onClick={() => {
           editor
            .chain()
