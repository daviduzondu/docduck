  ...
  orpc.documents.restoreSnapshotbyId.mutationOptions({
   onSuccess: () => {
    ...
    sendStateless(provider, {
     type: 'notify',
     data: 'Document reverted to older version',
    })
   },
  })
 )
...
function EditTitlePopover({
 ...
 return (
  <Popover>
   <PopoverTrigger
    className={'active:not-aria-[haspopup]:translate-y-px cursor-pointer p-0'}
   >
    {children}
   </PopoverTrigger>
   <PopoverContent
     ...
     <Controller
      ...
      render={({ field, fieldState }) => (
       <Field data-invalid={fieldState.invalid}>
        <FieldLabel htmlFor="title">Update Title</FieldLabel>
        <div className="flex space-x-2 w-full">
         <div className="flex-1 space-y-2">
          <Input {...field} id="title" placeholder="Update title..." />
          {fieldState.invalid && <FieldError errors={[fieldState.error]} />}
         </div>
         <Button className={'w-fit'} type="submit" form="update-title-form">
          Update
         </Button>
        </div>
       </Field>
...
function CollaboratorAvatar({
 ...
 return (
  <Avatar>
   ...
   >
    {collaborator.name?.split(' ')[0]![0]}
   </AvatarFallback>
  </Avatar>
 )
}

function CollaboratorsHoverCard({
 collaborators,
}: {
