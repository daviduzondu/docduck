...
export const generateAnonymousAvatar = (seed: string) =>
 createAvatar(thumbs, {
  seed,
 })

export function sendStateless<T = any>(
 provider: HocuspocusProvider,
 data: StatelessMessage<T>
) {
...
export function stripMarks(node: any, markNames = new Set()) {
 ...
 if (node.content && Array.isArray(node.content)) {
  return {
   ...node,
   content: node.content.map((child: any) => stripMarks(child, markNames)),
  }
 }
